...
async def test_list_file(client, valid_pdf, upload_files_headers):
    ...
    response = await client.get("/files", headers=upload_files_headers)
    assert response.status_code == 200
...
async def test_download_nonexistent_file(client, upload_files_headers):
    ...
    response = await client.get(
        f"/files/{non_existent_uuid}", headers=upload_files_headers
    )
...
async def test_delete_file(client, upload_files_headers, valid_pdf):
    ...
    response = await client.delete(f"/files/{file_id}", headers=upload_files_headers)
    assert response.status_code == 200
    assert response.json() is True

    response = await client.get(f"/files/{file_id}", headers=upload_files_headers)
    assert response.status_code == 404
