...
from server.utils.llm import get_langchain_llm
...
class Generator:
    ...
    def make_prompt(self, query: str, context: list[str]):
        if self.action == Action.default:
            return self.__default.format(query=query, documents="\n".join(context))
        elif self.action == Action.translate:
            return self.__translate.format(snippet=self.snippet)
        elif self.action == Action.explain:
            return self.__explain.format(
                snippet=self.snippet, documents="\n".join(context)
            )
        elif self.action == Action.ask:
            return self.__ask.format(
                query=query, snippet=self.snippet, documents="\n".join(context)
            )
        else:
            raise ValueError("None of action selected")

    async def __generate_stream(
    ...
    def __call__(self, query: str, context: list[str]) -> AsyncGenerator[str, None]:
